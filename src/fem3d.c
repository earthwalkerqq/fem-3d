...
void AssembleLocalStiffnessToGlobal3d(double** kglb, int** jt03, double** car,
  ...
  for (int elem = 0; elem < nelem; elem++) {
    ...
    printf(
        "Element %d coordinates: (%.2f,%.2f,%.2f), (%.2f,%.2f,%.2f), "
        "(%.2f,%.2f,%.2f), (%.2f,%.2f,%.2f)\n",
    ...
    for (int i = 0; i < 12; i++) {
      ...
      if (gest[i] == NULL) {
        printf(
            "Error: Cannot allocate row %d of local stiffness matrix for "
            "element %d\n",
            i, elem);
    ...
    nodeNumber3d node = {jt03[elem][0], jt03[elem][1], jt03[elem][2],
                         jt03[elem][3]};
